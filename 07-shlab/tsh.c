...
void do_bgfg(char **argv)
{
    ...
    if(!strcmp(argv[0], "fg")){
        ...
        // }
        // if((fg_pid = fgpid(jobs)) != 0 && fg_pid != job_ptr->pid){
        //     getjobpid(jobs, fg_pid)->state = BG;
        // }
        job_ptr->state = FG;
        waitfg(job_ptr->pid);
    }
...
void sigint_handler(int sig)
{
    ...
    if(fg_pid){
        ...
        kill(-fg_pid, SIGINT);// Kill the whole process group
        // deletejob(jobs, fg_pid);
    }
    else{
        printf("\nNo foregorund jobs to send SIGINT!\n");
    }
    return;
}
...
void sigtstp_handler(int sig)
{
    ...
    if(fg_pid){
        ...
        kill(-fg_pid, SIGTSTP);// Kill the whole process group
        // deletejob(jobs, fg_pid);
    }
    else{
        printf("\nNo foregorund jobs to send SIGTSTP!\n");
    }
    return;
}
